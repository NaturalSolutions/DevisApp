...
export class TaskService {
  ...
  getTasks(stories, projects, isFactu) {
    ...
    //     this.log.setlogProcess("Tasks have been parsed");
	 ...
	 getTasksSummary(stories, projects, isFactu, allUsers) {
		// this.log.setlogProcess('Getting Tasks from stories');
			let projetcts : Array<any> = [];
			let users : Array<any>;
     return new Promise<any>((resolve, reject) => {
			 users = [];
       let tasks = [];
       ...
       for (let i in projects) {
         ...
         if (stories != undefined) {
           ...
           for (let s in strfiltered) {
             ...
             let result = this.Angularget("https://www.pivotaltracker.com/services/v5/projects/" + strfiltered[s].project_id + "/stories/" + strfiltered[s].id + "/tasks")
               .toPromise().then((result) => {
								 storyOwners = strfiltered[s].owner_ids;
                 for (let u in result) {
                   if (result[u].story_id == strfiltered[s].id) {
										 tasks.push(result[u]);
										 projetcTasks.push(result[u]);
                     strfiltered[s].listeTaches.push(result[u]);
                   }
                 }
                 let projetid = projects.map(o => o.id);
                 ...
                 if (strfiltered[s].id != undefined && tasks != undefined && strfiltered[s].project_id != undefined) {
                  //  if (isFactu == true) {
										projetcTasks = this.getSummaryFromTasks(projetcTasks, strfiltered[s].id, projects[i].id, false,storyOwners, allUsers);
                  //    listeModifie = this.getInfoFromTasks(tasks, strfiltered[s].id, strfiltered[s].project_id, true);
                  //  } else {
                  //    listeModifie = this.getInfoFromTasks(tasks, strfiltered[s].id, strfiltered[s].project_id, false);
									//  }
									// this.
									console.log('count ',projetcTasks);
									this.globals.addAllToArray(projetcTasks)
									//projectTasksList = this.getSummaryFromTasks(tasks, strfiltered[s].id, projects[i].id, false,storyOwners, allUsers);
									if(listeModifie.length == 0 ){
										listeModifie = projetcTasks;	
									}else{
										listeModifie.concat(projetcTasks)
									}
									//listeModifie = this.getSummaryFromTasks(projetcTasks, strfiltered[s].id, projects[i].id, false,storyOwners, allUsers);
                 }
							 });
							 promises.push(result);
							}
						...
						// 	projects[i].tasks = projectTasksList;
						// 	this.summary.push(projects[i]);
						// }
       }
       Promise.all(promises).then(() => {
         let objectToSend: any = {};
         objectToSend.Taches = listeModifie;
		//     this.log.setlogProcess("Tasks have been parsed");
				console.log('tasks alla parsed', listeModifie)
         resolve(objectToSend);
       })
     });
   }
   ...
   getInfoFromTasks(tasks, storyId, projectId, isFactu): any {
	...
	getSummaryFromTasks(tasks, storyId, projectId, isFactu, storyOwners, users): any {
		var tasksModified = []; 							//initialisation tableaux vide
		var cpt = 0;		
		for (let i in tasks) {
			var tabDescrInfo = tasks[i].description.split('.-');
					...
					//Tache solo
						...
						if (tabDescrInfo[1].trim().match(regex)) {							
							...
							if(storyOwners.length > 1){
								let tempUsers = [];
								for(let u in storyOwners){
									tempUsers.push(users.find(o => o.id == storyOwners[u]));
								}
								// debugger
								if(tempUsers && tempUsers[0] != undefined){
									tasks[i].initales = tempUsers.map(o => o.initials).join('+');

								}
							}else{
								// debugger
								if(storyOwners[0]){
									tasks[i].initales = users.find(o => o.id == storyOwners[0]) ? users.find(o => o.id == storyOwners[0]).initials : 'CV';
								}
							}
							// this.log.setlogMessage('Probleme d\'initales dans la tâche : ' + tasks[i].id + ' de la storie n° : ' + tasks[i].story_id + ' n\'est attribué.\r\n https://www.pivotaltracker.com/n/projects/' + projectId + '/stories/' + tasks[i].story_id + '/tasks/' + tasks[i].id)
							// this.setError('https://www.pivotaltracker.com/n/projects/' + projectId + '/stories/' + tasks[i].story_id + '/tasks/' + tasks[i].id, tasks[i].id);
